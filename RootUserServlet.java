...
public class RootUserServlet extends HttpServlet {
        ...
        if (sqlStatement == null || sqlStatement.trim().isEmpty()) {
            ...
            try {
                ...
                if (isQuery) {
                    ...
                    try{
                        if(sqlStatement.toLowerCase().contains("shipment")){
                            message += "Business logic detected! - Updating supplier status<br>";
                            if(sqlStatement.toLowerCase().contains("insert") || sqlStatement.toLowerCase().contains("update") || sqlStatement.toLowerCase().contains("replace")){
                                sqlStatement = 
                                """
                                update suppliers
                                set status = status + 5
                                ...
                                message += "Business logic updated " + statement.getUpdateCount() + " supplier status marks.";
                            }
                        }
                        else{
                            if(rowsAffected == 0){
    ...
    private void getDBConnection() {
        ...
        try {
            FIN = new FileInputStream("C:/Program Files/Apache Software Foundation/Tomcat 11.0/webapps/Project4/WEB-INF/classes/root.properties");
            properties.load(FIN);
